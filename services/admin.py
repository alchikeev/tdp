...
class ServiceAdmin(admin.ModelAdmin):
    list_display = ('title', 'supplier', 'get_categories', 'price_adult', 'price_child', 'price_extra', 'is_active', 'is_popular', 'rating', 'reviews_count', 'created_at')
    list_filter  = ('is_active', 'is_popular', 'categories', 'tags')
    search_fields = ('title', 'short_desc', 'description', 'location')
    prepopulated_fields = {'slug': ('title',)}
    inlines = [ServiceImageInline]
    fieldsets = (
        (None, {'fields': ('title', 'supplier', 'slug', 'categories', 'tags', 'short_desc', 'description', 'location', 'youtube_url', 'cover')}),
